...
class RoomResourceIT {
    ...
    @Test
    @Transactional
    void checkMaxParticipantsIsRequired() throws Exception {
        long databaseSizeBeforeTest = getRepositoryCount();
        // set the field null
        room.setMaxParticipants(null);
    ...
    void putWithIdMismatchRoom() throws Exception {
        ...
        restRoomMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(roomDTO))
            )
            .andExpect(status().isBadRequest());
