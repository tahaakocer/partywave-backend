  ...
  return (
    <div>
      <h2 id="app-user-heading" data-cy="AppUserHeading">
        <Translate contentKey="partywaveApp.appUser.home.title">App Users</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="partywaveApp.appUser.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/app-user/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="partywaveApp.appUser.home.createLabel">Create new App User</Translate>
          </Link>
        </div>
      </h2>
        ...
        {appUserList && appUserList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="partywaveApp.appUser.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('spotifyUserId')}>
                  <Translate contentKey="partywaveApp.appUser.spotifyUserId">Spotify User Id</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('spotifyUserId')} />
                </th>
                ...
                <th className="hand" onClick={sort('url')}>
                  <Translate contentKey="partywaveApp.appUser.url">Url</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('url')} />
                </th>
                ...
                <th>
                  <Translate contentKey="partywaveApp.appUser.appUserStats">App User Stats</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {appUserList.map((appUser, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{appUser.url}</td>
                  <td>{appUser.type}</td>
                  <td>{appUser.ipAddress}</td>
                  <td>{appUser.lastActiveAt ? <TextFormat type="date" value={appUser.lastActiveAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
                    <Translate contentKey={`partywaveApp.AppUserStatus.${appUser.status}`} />
                  </td>
                  <td>
                    {appUser.appUserStats ? <Link to={`/app-user-stats/${appUser.appUserStats.id}`}>{appUser.appUserStats.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="partywaveApp.appUser.home.notFound">No App Users found</Translate>
            </div>
          )
        )}
      </div>
