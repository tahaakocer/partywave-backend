...
import com.partywave.backend.service.mapper.AppUserMapper;
...
/**
 * Service Implementation for managing {@link com.partywave.backend.domain.AppUser}.
 */
@Service
@Transactional
public class AppUserService {
    ...
    /**
     *  Get all the appUsers where UserToken is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<AppUserDTO> findAllWhereUserTokenIsNull() {
        LOG.debug("Request to get all appUsers where UserToken is null");
        return StreamSupport.stream(appUserRepository.findAll().spliterator(), false)
            .filter(appUser -> appUser.getUserToken() == null)
            .map(appUserMapper::toDto)
     ...
     */
    @Transactional(readOnly = true)
    public Optional<AppUserDTO> findOne(UUID id) {
