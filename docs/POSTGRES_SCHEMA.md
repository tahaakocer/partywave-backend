...
Represents a user's membership in a room. Uses soft delete pattern to preserve history.
...
- `joined_at: Instant` – When the user first joined this room.
- `last_active_at: Instant` – Last activity in this room (updated when user leaves or is active).
- `is_active: Boolean` – Whether the user is currently an active member (`true`) or has left (`false`). Default: `true`.
...
- `(room_id, app_user_id)` is logically unique (one membership record per user per room).
- **Soft delete pattern**: When users leave, `is_active` is set to `false` instead of deleting the record.
- **Active member queries**: All member count queries must filter by `is_active = true` to count only active members for room capacity checks.
- **Reactivation**: When a user rejoins a room they previously left, the existing membership record is reactivated (set `is_active = true`, update `last_active_at`) instead of creating a duplicate.
- `joined_at` timestamp is preserved across reactivations to maintain accurate join history.
- `last_active_at` tracks when the user last left or was last active in the room.
